...
const args = [
    ...
    '-cp', 'vendor/closure-compiler/compiler.jar' + classPathSep + 'vendor/ng-closure-runner/ngcompiler.jar',
...
execFile(javaExecutable, args, function(error, stdout, stderr) {
    if (error) {
        grunt.fail.warn('Error minifying ' + file + ': ' + stderr);
        return;
    }

    // Process the output files as before
